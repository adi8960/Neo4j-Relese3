...
import { stronglyConnectedComponents } from 'webcola';
...
export class InstanceView extends GraphView {
    constructor(container, graphService, visibilityService, urlService) {
        ...
        //For Sending of Particular nodes of data to other instance of knowledge
        var comb = {};
        var selected = [];
        var nodeset = new Set();
        var edgeset = new Set();
        let url = 'http://localhost:8001';
        $.getJSON(url + "/export/_graph", (exportgraph) => {
            for (var i = 0; i < exportgraph.nodes.length; i++) {
                //Pushing the entire schema of the schema pane into the final object variable(Only-Once-For-All-Selection/DeSelection)
                if (this.isInArray("Class", exportgraph.nodes[i].t_labels)) {
                    nodeset.add(exportgraph.nodes[i]);
                };
            };
            for (var i = 0; i < exportgraph.edges.length; i++) {
                if (exportgraph.edges[i].r_type === "isa") {
                    edgeset.add(exportgraph.edges[i]);
                };
            };
        });
        this.initialLayout = {
        ...
        //this is for selecting the particular edge upon expansion of the instance for sending
        this.cy.cxtmenu({
            selector: 'edge',
            activePadding: 2,
            commands: (nele) => {
                ...
                let cmds = []
                cmds = [...cmds, mkCtxCommand((ele) => {
                    console.log("Inside edge cxt menu", ndata);
                    // console.log("!.----0",ndata.props._oid);
                    $.getJSON(url + "/export/_graph", (exportgraph) => {
                        //adding the edge which is connected to a particular node

                        for (var i = 0; i < exportgraph.nodes.length; i++) {
                            if (exportgraph.nodes[i].t_oid === ndata.props._oid) {
                                nodeset.add(exportgraph.nodes[i]);
                            };
                        };

                        for (var j = 0; j < exportgraph.edges.length; j++) {
                            if (exportgraph.edges[j].r_start === ndata.props._oid && exportgraph.edges[j].r_type === "instance_of") {
                                edgeset.add(exportgraph.edges[j]);
                            }
                        }
                        alert("You have just selected an edge: " + ndata.cls);


                        comb["nodes"] = Array.from(nodeset);
                        comb["edges"] = Array.from(edgeset);
                        console.log("combined", comb);

                    });
                }, 'Select Edge', 'fa-arrow-left', true),
        ...
        this.cy.cxtmenu({
            ...
            commands: (nele) => {
                ...
                //::This is for creating Select Node and Deselect Node Context Menu Option::
                mkCtxCommand((ele) => {
                    //Pushing name of the selected node instance into the Display menu object(selected).
                    selected.push(ele.data().display);
                    console.log("Element", ele);
                    $.getJSON(url + "/export/_graph", (exportgraph) => {
                        for (var i = 0; i < exportgraph.nodes.length; i++) {
                            if (exportgraph.nodes[i].t_oid === ele.data().oid) {
                                nodeset.add(exportgraph.nodes[i]);
                            };
                        };
                        for (var i = 0; i < exportgraph.edges.length; i++) {
                            if (exportgraph.edges[i].r_start === ele.data().oid || exportgraph.edges[i].r_end === ele.data().oid) {
                                edgeset.add(exportgraph.edges[i]);
                            };

                        };

                        console.log("Nodeset", Array.from(nodeset));
                        console.log("edgeset", Array.from(edgeset));
                        comb["nodes"] = Array.from(nodeset);
                        comb["edges"] = Array.from(edgeset);
                    });
                    window.CombinedObject = comb;//displaying the selected object
                    console.log("Window combined object", window.CombinedObject);
                    console.log("In Selection", selected);
                    alert("You have just selected a Node: " + ele.data().display);
                    //Updating the Display Menu Selected Object after every select 
                    document.getElementById("nodes1").innerHTML = selected
                }, 'Select Node', 'glyphicon glyphicon-hand-up', true),

                mkCtxCommand((ele) => { //deselection menu
                    // //Removal of name from the Display Menu Object
                    for (var i = 0; i < selected.length; i++) {
                        if (ele.data().display === selected[i]) {
                            selected.splice(i, 1);
                        }
                    }

                    for (var i = 0; i < comb.nodes.length; i++) {
                        if (comb.nodes[i].t_oid === ele.data().oid) {
                            comb.nodes.splice(i, 1);
                        }
                    };
                    for (var i = 0; i < comb.edges.length; i++) {
                        if (comb.edges[i].t_oid === ele.data().oid || comb.edges[i].r_end === ele.data().oid || comb.edges[i].r_start === ele.data().oid) {
                            comb.edges.splice(i, 1);
                        };
                    };
                    window.CombinedObject = comb;
                    console.log("selection", selected);
                    console.log("Window combined object of Deselct", window.CombinedObject);
                    console.log("In Selection of deslect Option menu", selected);
                    //Updating the Display Menu Selected Object after every select 
                    document.getElementById("nodes1").innerHTML = selected
                }, 'DeSelect Node', 'glyphicon glyphicon-hand-down', true),
                ];
                return cmds;
            }
